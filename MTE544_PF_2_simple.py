...
dx = 0.1
dy = 0.1
# we limit the number of particles
NP = 1000
...
while time <= 3:
    ...
    for row_index, row in enumerate(pf_map_fixed):
        for col_index, lis in enumerate(row):
            for elem_index, elem in enumerate(lis):
                if elem[3] > np.random.normal(0, 1, 1):  # particle weight boundary was here
...
ax = plt.axes()
ax.plot(robot_path_x[1:], robot_path_y[1:], label='True Path')
ax.plot(estimate_path_x[1:], estimate_path_y[1:], label='Estimated Path')
plt.title('Final Path(s)')
plt.legend()
plt.show()
